import {AnimatedSprite, Sprite} from "pixi.js/dist/browser/pixi.min.mjs";
import Helper from '../utils/helper';

export class AnimatedParticle extends AnimatedSprite {
  constructor(emitter, data) {
    super(data.texture);
    super.animationSpeed = 0.3;
    super.loop = true;
  ...
  reset(data) {
    ...
    const rndFrame = Math.floor(Math.random() * texture.length);
    const tex = texture.slice(rndFrame + 1, -1).concat(texture.slice(0, rndFrame));
    this.textures = tex;
    this.anchor.set(0.5);
